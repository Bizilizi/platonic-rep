...
model.model.mm_projector.register_forward_hook(hook_fn)
...
for i in tqdm(indices):
    try: 
        ...
        torch.save(hidden_states.mean(dim=1), activations_path / "image_hidden_states_avg.pt")
        torch.save(hidden_states[:, -1].clone(), activations_path / "image_hidden_states_last.pt")
        
        # save visual_encoder activations
        torch.save(activations.mean(dim=0), activations_path / "visual_encoder_activations_avg.pt")
        torch.save(activations[-1].clone(), activations_path / "visual_encoder_activations_last.pt")
        
        # save text hidden states 
        inputs = processor(
            text=sample["context_page_description"],
            add_system_prompt=True,
            add_generation_prompt=True,
            return_tensors="pt" 
        )
        ...
        torch.save(hidden_states.mean(dim=1), activations_path / "text_hidden_states_avg.pt")
        torch.save(hidden_states[:, -1].clone(), activations_path / "text_hidden_states_last.pt")
