...
def extract_llm_features(filenames, dataset, args):
    ...
    texts = [
        str(x["text"][args.caption_idx])
        for x in dataset[args.page_size * args.page : args.page_size * (args.page + 1)]
    ]
    ...
    for llm_model_name in filenames[::-1]:
        ...
        language_model = load_llm(
            llm_model_name, qlora=args.qlora, force_download=args.force_download
        )
